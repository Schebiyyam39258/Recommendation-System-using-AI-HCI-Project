...
books = pd.read_csv("C:/datasets/Books.csv", encoding='latin-1', on_bad_lines='skip')

# Selecting only book title and author columns and removing missing or duplicate entries
books = books[['Book-Title', 'Book-Author']].dropna().drop_duplicates()
...
books['content'] = books['Book-Title'].astype(str) + " " + books['Book-Author'].astype(str)
...
print(books['Book-Title'].sample(10).to_string(index=False))
